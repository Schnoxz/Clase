...
public abstract class CentroOperativo implements Supervisable {
    ...
    public CentroOperativo(String codigo, Zona zona, Responsable responsable, int[][] operaciones,
            int[][] incidencias) {
        this.codigo = codigo;
        this.zona = zona;
    ...
    public int calcularOperacionesDia(int dia) {
        ...
        for (int turno = 0; turno < operaciones[dia].length; turno++) {
            totalOperaciones += operaciones[dia][turno];
        }
    ...
    public int calcularIncidenciasDia(int dia) {
        ...
        for (int turno = 0; turno < incidencias[dia].length; turno++) {
            totalIncidencias += incidencias[dia][turno];
        }
    ...
    public double calcularTasaIncidencias() {
        return ((calcularTotalIncidencias() * 100.0) / calcularTotalOperaciones());
    }
